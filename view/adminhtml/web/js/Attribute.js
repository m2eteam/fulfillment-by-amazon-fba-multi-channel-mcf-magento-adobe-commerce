define([], function () {
    window.Attribute = Class.create({
        ...
        checkAttributesSelect: function (id, value) {
            if ($(id)) {
                if (typeof AmazonMcf.formData[id] != 'undefined') {
                    $(id).value = AmazonMcf.formData[id];
                }
                if (value) {
                    $(id).value = value;
                }
        ...
        renderAttributesWithEmptyHiddenOption: function (id, insertTo, value, width) {
            var style = width ? ' style="width: ' + width + 'px;"' : '';
            var txt = '<select name="' + id + '" id="' + id + '" class="AmazonMcf-required-when-visible select admin__control-select"' + style + '>\n';
        ...
        renderAttributesWithEmptyOption: function (id, insertTo, value, notRequiried) {
            var classes = 'AmazonMcf-custom-attribute-can-be-created select admin__control-select';

            if (!notRequiried) {
                classes += ' AmazonMcf-required-when-visible';
            }
